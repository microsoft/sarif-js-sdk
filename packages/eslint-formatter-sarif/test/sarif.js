...
describe('formatter:sarif', () => {
  describe('when passed one message', () => {
    ...
    it('should return a log with one file and one result', () => {
      ...
      assert.strictEqual(
        log.runs[0].results[0].locations[0].physicalLocation.artifactLocation.index,
        0
      );
    });
  });
...
describe('formatter:sarif', () => {
  describe('when passed one message with line, column, and source string', () => {
    ...
    it('should return a log with one result whose location contains a region with line and column #s', () => {
      ...
      assert.strictEqual(
        log.runs[0].results[0].locations[0].physicalLocation.region.snippet.text,
...
describe('formatter:sarif', () => {
  describe('when passed two results with two messages each', () => {
    ...
    it('should return a log with two files, three rules, and four results', () => {
      ...
      assert.strictEqual(log.runs[0].tool.driver.rules[1].id, ruleid2);
      assert.strictEqual(
        log.runs[0].tool.driver.rules[1].shortDescription.text,
        rule2.docs.description
      );
      ...
      assert.strictEqual(log.runs[0].tool.driver.rules[2].id, ruleid3);
      assert.strictEqual(
        log.runs[0].tool.driver.rules[2].shortDescription.text,
        rule3.docs.description
      );
      ...
      assert.strictEqual(
        log.runs[0].results[1].locations[0].physicalLocation.artifactLocation.index,
        0
      );
      assert.strictEqual(
        log.runs[0].results[2].locations[0].physicalLocation.artifactLocation.index,
        1
      );
      assert.strictEqual(
        log.runs[0].results[3].locations[0].physicalLocation.artifactLocation.index,
        1
      );
      ...
      assert.strictEqual(
        log.runs[0].results[1].locations[0].physicalLocation.region.startColumn,
        5
      );
      assert.strictEqual(
        log.runs[0].results[1].locations[0].physicalLocation.region.snippet.text,
        'doSomething(thingId)'
      );
